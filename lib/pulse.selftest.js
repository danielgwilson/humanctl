...
const { extractIssueKeys } = require('./sessions');
...
check('unattached and out-of-scope sessions land in diagnostics', () => {
  const collected = emptyCollected({
    gitRepos: [{ name: 'acme', github: 'example/acme', path: '/home/dev/work/acme/main', degraded: null, worktrees: [worktree('main', { isPrimary: true })] }],
    sessions: {
      degraded: null,
      rows: [
        session('s-5', '/home/dev/work/acme/main', { inScope: true, lastRole: 'assistant' }),
  ...
  assert.ok(unattached && unattached.sessions === 1 && unattached.needsYou === 1);
