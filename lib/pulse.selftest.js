...
// ---- ask-the-session probes: sentinel turns must be invisible to the reader ----
// Persisted probe turns ("[humanctl btw] ..." plus the answer the model gave)
// appear in Codex rollouts on every ask, and would appear in Claude transcripts
// on a future fork path. The reader must pair-drop the whole probe exchange:
...
check('codex btw probe never flips state, resets the idle clock, or masks the ask', () => {
  ...
  const probe = [
    codexEv(T_PROBE, { type: 'user_message', message: '<environment_context>synthetic wrapper</environment_context>' }),
    codexEv(T_PROBE, { type: 'user_message', message: `${BTW_SENTINEL} status?` }),
    codexEv(T_PROBE, { type: 'message', role: 'user', content: [{ text: `${BTW_SENTINEL} status?` }] }),
    codexEv(T_PROBE, { type: 'reasoning', summary: [{ text: 'probe thinking' }] }),
    codexEv(T_PROBE, { type: 'agent_message', message: 'Working through the adapter; nothing blocking.' }),
  ];
  const before = signalsAndState(tmpTranscript('rollout-synthetic-a.jsonl', genuine), 'codex');
  const after = signalsAndState(tmpTranscript('rollout-synthetic-b.jsonl', genuine.concat(probe)), 'codex');
  assert.strictEqual(before.need.state, 'need');
  assert.strictEqual(after.need.state, 'need', 'probe flipped the state');
  assert.strictEqual(after.need.reason, before.need.reason, 'probe changed the reason');
  assert.strictEqual(after.sig.lastActiveMs, Date.parse(T_ASK), 'probe refreshed lastActiveMs');
  assert.strictEqual(after.sig.lastAssistantText, before.sig.lastAssistantText, 'probe answer replaced the classified tail');
  assert.strictEqual(after.sig.lastUserText, 'wire the adapter please', 'sentinel leaked into lastUserText');
  assert.strictEqual(after.sig.userCount, before.sig.userCount, 'probe counted as a user turn');
  assert.strictEqual(after.sig.toolCount, before.sig.toolCount, 'probe tool noise counted');
});

check('probe answer cannot fabricate an ask on a completed session', () => {
  const genuine = [
    codexEv(T_USER, { type: 'user_message', message: 'ship the release' }),
    codexEv(T_ASK, { type: 'agent_message', message: 'Release tagged, merged and shipped.' }),
  ];
  const probe = [
    codexEv(T_PROBE, { type: 'user_message', message: `${BTW_SENTINEL} anything left?` }),
    codexEv(T_PROBE, { type: 'agent_message', message: 'Want me to draft the follow-up plan?' }),
  ];
  const after = signalsAndState(tmpTranscript('rollout-synthetic-c.jsonl', genuine.concat(probe)), 'codex');
  assert.strictEqual(after.need.state, 'done', 'ask-shaped probe answer minted a needs-you');
});

check('claude btw probe turns are pair-dropped the same way', () => {
  const genuine = [
    claudeEv(T_USER, 'user', 'backfill the export manifest'),
    claudeEv(T_ASK, 'assistant', [{ type: 'text', text: 'Manifest is staged. Keep the legacy columns or drop them?' }]),
  ];
  const probe = [
    claudeEv(T_PROBE, 'user', `${BTW_SENTINEL} quick status check`),
    claudeEv(T_PROBE, 'assistant', [{ type: 'text', text: 'Staging held; still waiting on the column decision.' }]),
  ];
  const before = signalsAndState(tmpTranscript('claude-synthetic-a.jsonl', genuine), 'claude-code');
  const after = signalsAndState(tmpTranscript('claude-synthetic-b.jsonl', genuine.concat(probe)), 'claude-code');
  assert.strictEqual(before.need.state, 'need');
  assert.strictEqual(after.need.state, 'need');
  assert.strictEqual(after.need.reason, before.need.reason);
  assert.strictEqual(after.sig.lastActiveMs, Date.parse(T_ASK));
  assert.strictEqual(after.sig.lastAssistantText, before.sig.lastAssistantText);
  assert.strictEqual(after.sig.lastUserText, 'backfill the export manifest');
});

check('a genuine user turn after a probe closes the skip window', () => {
  const lines = [
    codexEv(T_USER, { type: 'user_message', message: 'wire the adapter please' }),
    codexEv(T_ASK, { type: 'agent_message', message: 'Adapter is wired. Should the debounce be 2s or 5s?' }),
    codexEv(T_PROBE, { type: 'user_message', message: `${BTW_SENTINEL} status?` }),
    codexEv(T_PROBE, { type: 'agent_message', message: 'Waiting on the debounce call.' }),
    codexEv('2026-01-15T17:55:00.000Z', { type: 'user_message', message: 'go with 5s' }),
    codexEv('2026-01-15T17:56:00.000Z', { type: 'agent_message', message: 'Done: debounce set to 5s, merged and shipped.' }),
  ];
  const { sig, need } = signalsAndState(tmpTranscript('rollout-synthetic-d.jsonl', lines), 'codex');
