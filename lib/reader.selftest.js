...
const { readTimelinePage, readAppended, primeTailCursor, readBlocks, readNotes, listRecent } = require('./sessions');
...
// ---- HOME is re-resolved per call, not frozen at require time ----
check('readNotes and listRecent follow a HOME swap made after require', () => {
  ...
  try {
    ...
    assert.ok(rows.length >= 1, 'scan sees the fixture session under the swapped home');
    assert.ok(rows.every((r) => r.path.startsWith(home)), 'scan never leaks sessions from the real home');
  } finally {
