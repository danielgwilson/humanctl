...
//   (worktree paths and branch names the tail actually mentions), because
//   humans launch sessions from parent directories while agents work inside
//   child repos via `git -C`. Evidence joins are tagged inferred, never
//   presented as explicit.
...
// Last-edit probe for worktrees whose branch tip is old. Commit age alone
// misses live worktrees: an agent can edit files for hours without committing,
// and binning that worktree as stale is a false signal (the ground-truth
// audit caught exactly this). The probe must stay cheap on real fleets:
// sampling tracked-file mtimes via `git ls-files` measured 103ms per worktree
// across a 658-worktree fleet (+6s per run, over the whole warm budget), so
// the probe reads two filesystem timestamps instead, no subprocess:
// - the worktree's own gitdir index mtime: every index-touching git command
//   run in that worktree (status, add, checkout, commit) updates it, and
//   agent sessions run git constantly while they edit
// - the worktree root directory mtime: file creation or deletion at the root
...
function worktreeEditMtime(wtPath, now) {
  ...
  try {
    ...
    if (st.isDirectory()) {
      ...
      // Linked worktree: .git is a one-line file pointing at the real gitdir.
      const m = /^gitdir:\s*(.+?)\s*$/m.exec(fs.readFileSync(dotGit, 'utf8'));
      if (m) indexPath = path.join(path.resolve(wtPath, m[1]), 'index');
    }
    if (indexPath) {
      try {
        const im = fs.statSync(indexPath).mtimeMs;
        if (im > newest) newest = im;
      } catch { /* no index yet: fall through to the dir mtime */ }
    }
    const dm = fs.statSync(wtPath).mtimeMs;
...
async function collectGitRepo(repo, staleHours, now) {
  ...
  // status` in each would dominate the run. A worktree whose ACTIVITY (last
  // commit or newest sampled file edit, whichever is later) is far past the
  // stale window is stale by no-movement regardless of its tree state, so the
  // dirty check is skipped there and reported as null (unknown), never
  // fabricated. The mtime probe is what keeps a recently-edited dirty worktree
  // out of that skip: uncommitted edits count as movement. Primaries always
  // get the status check: a dirty main checkout is one of the signals pulse
  // exists to surface.
  const staleMs = staleHours * 3.6e6;
  ...
  await mapLimit(worktrees, 12, async (w) => {
    ...
    // Only probe when the commit age alone would read as stale: a fresh tip
...
function collectSessions(config, gitRepos) {
  ...
  for (const r of rows) {
    ...
    // Parent-dir launches: the human starts the session at the workspace root
    // and the agent works inside a child repo via `git -C`. A cwd that is an
    // ancestor of a configured repo path makes the session a join CANDIDATE;
    // the actual join still requires transcript evidence (issue key, repo or
    // worktree path, branch name), so sibling work under the same parent dir
    // never joins a repo it did not touch.
    r.ancestorScope = !r.inScope && config.repos.some((repo) => cwdWithin(repo.path, r.cwd));
    if (r.inScope || r.ancestorScope) {
      ...
      // Bounded tail evidence scan, skipped when cwd already sits inside a
      // non-primary worktree (containment is a stronger join than evidence).
      const contained = nonPrimaryRoots.some((root) => cwdWithin(r.cwd, root));
      r.workRefs = contained ? { roots: [], tokens: [] } : sessionsLib.readWorkRefs(r.path, { roots, tokens });
    } else {
...
// Session activity honesty: needs-you v3 rows carry lastActiveMs, the last
// SUBSTANTIVE event's own timestamp. File mtime overstates recency whenever
...
function reconcile(collected, config) {
  ...
  // 4) Attempts: sessions join, strongest signal first:
  //    a. issue keys in the session TITLE (a deliberate, human-visible label)
  //    b. cwd containment inside a discovered worktree
  //    c. transcript evidence: a worktree path or branch name the tail
  //       actually mentions (see collectSessions)
  //    d. corroborated issue keys in the transcript body, capped: bare tokens
  //       are noisy, so they only join when linear/git/gh already know the
  //       key AND the session names a few, not the whole board
  ...
  if (collected.sessions.rows) {
    ...
    // A transcript that name-drops many known keys is an orchestrator or a
    // ramp log, not a worker on all of those units. Joining every mention
    // fanned single sessions across 40+ stale branch units on the real fleet
    // and flipped them all into needs-you.
    const SESSION_KEY_FANOUT_CAP = 3;

    for (const s of collected.sessions.rows) {
      // a. title and cwd keys: deliberate, human-visible labels always join.
      ...
      // c. transcript evidence: deepest mentioned non-primary worktree first
      // (a path inside a worktree is specific in a way bare tokens are not),
      // then a mentioned branch name. Both target ONE unit; a session never
      ...
      for (const root of evRoots) {
        ...
        if (hit && !hit.w.isPrimary) {
          const u = units.find((x) => x.worktrees.includes(hit.w));
          if (u) { evUnit = u; evRepo = hit.repoName; break; }
        }
      }
      if (!evUnit) {
        for (const t of ev.tokens || []) {
          for (const repo of collected.gitRepos) {
            const u = unitByRepoBranch.get(`${repo.name}:${t}`);
            if (u) { evUnit = u; evRepo = repo.name; break; }
          }
          if (evUnit) break;
        }
      }
      ...
      // Repo-level join: cwd inside the repo, or (for ancestor-cwd candidates)
      ...
      if (!repoName) { outOfScope += 1; continue; }
      if (sessionWaits(s) || sessionWorking(s)) {
        // A live session on a known repo with no branch or PR unit mints a
        // session-evidence unit. Lane logic decides needs-you (the reader
        // says the agent is waiting on the human) vs missing-owner (the
        // agent is working with no reconciled issue). Done, idle, and
        // archived sessions land in diagnostics instead of minting noise.
        const u = localUnit(`session:${repoName}:${s.id}`);
        u.joins.push('inferred');
        u.sessions.push(s);
        u.repoNames.add(repoName);
        continue;
      }
      const cur = unattachedByRepo.get(repoName) || { sessions: 0 };
      cur.sessions += 1;
  ...
  for (const u of units) {
    ...
    // A needs-you session (per the reader's content-shaped state, see
    // sessionWaits above) flips the lane only when nothing on the unit moved
    // after it: newer sibling activity means the ball already moved on.
    const needsYouSession = u.sessions.find((s) =>
      sessionWaits(s)
      && !activity.some((t) => t > sessionActivityMs(s))
    ) || null;
    ...
    const item = {
      ...
      title: (u.issue && u.issue.title)
        ...
        || (u.sessions[0] && (u.sessions[0].customTitle || u.sessions[0].title))
        || u.id,
      ...
      executionRef: (u.worktrees.length || u.sessions.length)
        ? {
            ...
            sessions: u.sessions.slice(0, 5).map((s) => ({ id: s.id, harness: s.harness, lastRole: s.lastRole, state: s.state || null, stateReason: s.stateReason || null, age: relAge(sessionActivityMs(s), now) })),
          }
        : null,
  ...
  // Fresh unattached sessions mint their own units above, so what remains here
  // is decayed history, not hidden work.
  for (const [repoName, counts] of unattachedByRepo) {
    diagnostics.push({ type: 'unattached-sessions', repo: repoName, sessions: counts.sessions, reason: 'sessions on the repo (past the needs-you decay window) joined no issue, branch, or worktree unit' });
  }
  if (outOfScope > 0) diagnostics.push({ type: 'sessions-out-of-scope', count: outOfScope, reason: 'recent sessions with no cwd or transcript evidence tying them to a configured repo' });
