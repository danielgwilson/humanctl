...
function reconcile(collected, config) {
  ...
  if (collected.sessions.rows) {
    for (const s of collected.sessions.rows) {
      ...
      if (owner) {
        ...
        cur.sessions += 1;
        // content-shaped state when available; lastRole+decay fallback otherwise
        const waits = s.state
          ? (s.state === 'need' && s.tier !== 'archived')
          : (s.lastRole === 'assistant' && now - s.ageMs <= NEED_DECAY_MS);
        if (waits) cur.needsYou += 1;
  ...
  for (const u of units) {
    ...
    // after it: newer sibling activity means the ball already moved on. Rows
    // from the v3 reader carry a content-shaped state ('need' means the tail
    // actually asks for you); rows without one (older callers, synthetic
    // fixtures) fall back to the lastRole + decay heuristic.
    ...
    const item = {
      ...
      executionRef: (u.worktrees.length || u.sessions.length)
        ? {
            ...
            sessions: u.sessions.slice(0, 5).map((s) => ({ id: s.id, harness: s.harness, lastRole: s.lastRole, state: s.state || null, stateReason: s.stateReason || null, age: relAge(s.ageMs, now) })),
          }
        : null,
