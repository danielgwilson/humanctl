...
// HOME is resolved per call, never frozen at require time: selftests swap
// process.env.HOME to point the readers at fixture trees (os.homedir() reads
// $HOME on POSIX), and a require-time constant would pin every derived path
// to the real home forever. Same pattern as controlDir() in commands.js.
function harnesses() {
...
function baseScan(maxAgeH, limit, includeAutomation) {
  const home = os.homedir();
  // home is part of the key so a HOME swap (test isolation) never serves rows
  // scanned from the previous home within the TTL window.
  const key = `${home}:${maxAgeH}:${limit}:${!!includeAutomation}`;
...
const notesFile = () => path.join(os.homedir(), '.humanctl', 'notes.jsonl');
...
module.exports = {
  ...
  ISSUE_KEY_RE, KINDS, BTW_SENTINEL,
  NEED_DECAY_MS, TIER_HOT_MS, TIER_DRIFT_MS, FRESH_MS,
  // Getters, not snapshots: consumers reading these lazily (pulse.js) see the
  // current HOME. A require-time destructure still works but freezes the value.
  get NOTES_FILE() { return notesFile(); },
