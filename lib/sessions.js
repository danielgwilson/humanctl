...
function readNeedSignals(file, harness, st) {
  ...
  for (const ln of lines) {
    ...
    if (claude) {
      ...
      if (m.role === 'user') {
        if (Array.isArray(m.content) && m.content.some((x) => x && x.type === 'tool_result')) { events.push({ kind: 'tool', ts: tsv }); continue; }
        const t = genuineUserText(m.content).trim();
        if (!t) continue;
        if (INTERRUPT_RE.test(t)) { events.push({ kind: 'interrupt', ts: tsv }); continue; }
      ...
      if (pt === 'user_message' && p.message) {
        ...
        if (/<turn_aborted/.test(t)) { events.push({ kind: 'interrupt', ts: tsv }); continue; }
...
module.exports = {
  ...
  accountStatus, readNotes, extractIssueKeys, readIssueRefs, readNeedSignals,
  deriveNeedState, askShapeOf, doneShapeOf, isClaudeOneShot,
  ISSUE_KEY_RE, NOTES_FILE, HARNESSES, KINDS,
