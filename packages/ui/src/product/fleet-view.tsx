      ...
      {rows.map((row) => {
        ...
        return (
          <div key={row.key} className="relative grid min-h-[var(--row-decision)] grid-cols-[minmax(9rem,1fr)_minmax(12rem,2fr)_4rem_4rem] items-center gap-4 overflow-hidden border-b border-border px-4 max-[720px]:grid-cols-[minmax(8rem,1fr)_4rem]">
            <span className="relative text-sm font-medium text-ink">{row.label}</span>
            <span className="relative truncate text-xs text-ink-3 max-[720px]:hidden">{row.detail}</span>
            <span className="relative text-right text-xs tabular-nums text-ink">{row.count}</span>
            <span className="relative text-right text-xs tabular-nums text-ink-3 max-[720px]:hidden">{share}%</span>
    ...
    <PageFrame>
      ...
      <PageBody>
        <ScrollArea className="h-full">
            ...
            <div className="grid grid-cols-5 border-b border-border max-[760px]:grid-cols-2">
              {STATE_ORDER.map((state, index) => (
                <div key={state} className="border-r border-border px-4 py-4 last:border-r-0 max-[760px]:border-b">
                  <div className="text-xs font-medium text-ink-3">{state === "need" ? "Needs you" : state === "block" ? "Blocked" : state === "work" ? "Working" : state === "done" ? "Complete" : "Idle"}</div>
                  <div className="mt-1 flex items-baseline gap-2"><span className="text-[24px] leading-7 font-semibold tabular-nums text-ink">{counts[state]}</span>{index === 0 && status?.needsYou ? <span className="text-xs text-need">{status.needsYou === 1 ? "action" : "actions"}</span> : null}</div>
                </div>
              ))}
            </div>
            {status?.nearCompaction ? (
              <div className="flex min-h-10 items-center gap-2 border-b border-need/30 bg-need-soft px-4 text-sm text-need"><AlertTriangleIcon className="size-3.5" />{status.nearCompaction} {status.nearCompaction === 1 ? "session is" : "sessions are"} near compaction.</div>
            ) : null}
          </section>
          ...
          <div className="flex min-h-10 items-center border-b border-border px-4 text-xs text-ink-3">
