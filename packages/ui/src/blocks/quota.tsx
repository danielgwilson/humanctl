      ...
      <div className="min-w-24 text-right">
        {loading ? <Skeleton className="ml-auto h-4 w-12" /> : (
          normalized == null ? <span className="text-xs text-ink-3">N/A</span> : (
            <>
              <span className="block text-xs tabular-nums text-ink">{Math.round(normalized)}% used</span>
              <span className="block text-xs tabular-nums text-ink-3">{Math.round(100 - normalized)}% remaining</span>
            </>
          )
        )}
      </div>
