...
// No scrim (section 4, same reasoning as dialog.tsx's DialogOverlay): the
// Radix Overlay element stays mounted, transparent, for click-outside and
// aria-hidden -- the drawer earns its separation from `overlay` elevation
// alone.
  ...
  return (
    <SheetPortal>
      ...
      <SheetPrimitive.Content
        ...
        className={cn(
          "fixed z-50 flex flex-col gap-4 bg-surface-2 overlay transition ease-in-out data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:animate-in data-[state=open]:duration-500",
          side === "right" &&
...
function SheetTitle({
  ...
  return (
    <SheetPrimitive.Title
      ...
      className={cn("font-mono text-row text-ink", className)}
      {...props}
    />
  )
}
...
function SheetDescription({
  ...
  return (
    <SheetPrimitive.Description
      ...
      className={cn("font-mono text-micro text-ink-3", className)}
      {...props}
    />
  )
}
