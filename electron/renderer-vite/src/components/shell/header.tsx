...
// Ported from index.html's .hdr: slim header, wordmark + version + the
// sidebar-toggle icon only (DESIGN.md signal-ownership: digest/resources/
// theme controls do NOT live here). The Tooltip on the icon button is a
// Radix primitive replacing the old [data-tip] pure-CSS tooltip; same visual
// contract (delayed show, dismiss on Esc/blur) but keyboard focus and ARIA
// wiring (role="tooltip", aria-describedby) come from Radix for free.
export function Header({ demo, rightRailOpen, onToggleRightRail }: { demo: boolean; rightRailOpen: boolean; onToggleRightRail: () => void }) {
  return (
    <header
    ...
    >
      <div className="flex flex-none items-baseline gap-2" style={{ WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
        ...
        <span className="font-mono text-[9px] uppercase tracking-widest text-ink3">vite spike</span>
        {demo && (
